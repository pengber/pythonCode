...
def zmMinFilterGray(src, r=7):#这个半径对于去雾效果是有影响的。一定情况下，半径越大去雾的效果越不明显，建议的范围一般是5-25之间，一般选择5,7,9等就会取得不错的效果。
    '''最小值滤波，r是滤波器半径'''
...
def Defog(m, r, eps, w, maxV1):                 # 输入rgb图像，值范围[0,1]
    ...
    V1 = np.min(m, 2)                           # 取得灰度图
    Dark_Channel = zmMinFilterGray(V1, 7)       # 最小值滤波得到暗通道图像
...
if __name__ == '__main__':
    m = deHaze(cv2.imread('dehaze_test.png') / 255.0) * 255
    cv2.imwrite('lena_dehaze.tif', m)
